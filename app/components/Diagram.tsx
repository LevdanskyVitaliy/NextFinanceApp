...
//       // SINCE category.id IS NUMBER: Use direct number comparison
...
export default function CircularDiagram() {
  ...
  const categoryData = useMemo<CategoryData[]>(() => {
    ...
    filteredTransactions.forEach((transaction) => {
      const categoryName = String(transaction.category || "").trim() || "Uncategorized";
      
      const category = categories.find((c) => 
        c.name.toLowerCase() === categoryName.toLowerCase()
      );
      
      const finalCategoryName = category ? category.name : categoryName;
      
      const amount = Math.abs(transaction.amount || 0);

      categoryMap.set(
        finalCategoryName,
        (categoryMap.get(finalCategoryName) || 0) + amount
      );
    });
