...
export default function EditForm() {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (!tx) {
          ...
          console.log("category", formattedTransaction.category);
  ...
  return (
    <div className="max-w-4xl sm:p-6 p-2 border border-gray-300 dark:border-gray-600 dark:text-white bg-[#f5f6fb] dark:bg-[#0c1017] rounded-lg shadow-md shadow-gray-500 dark:shadow-gray-800">
      <Item variant="muted">
        <ItemContent>
          <ItemTitle className="border-b-2 md:text-xl sm:text-lg text-md font-bold sm:pb-2 pb-1">
          ...
          <form onSubmit={handleSubmit} className="sm:space-y-6 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-6 gap-3">
              <div className="sm:space-y-6 space-y-3">
                <div>
                  <Label htmlFor="type" className="text-sm font-medium text-gray-500 dark:text-gray-400 capitalize border-b-2 block pb-1">
                    Type
                  </Label>
                  <Select
                    value={formData.type}
                ...
                <div>
                  <Label htmlFor="amount" className="text-sm font-medium text-gray-500 dark:text-gray-400 capitalize border-b-2 block sm:pb-2 pb-1">
                    Amount
                  </Label>
                  <Input
                    type="number"
                ...
                <div>
                  <Label htmlFor="category" className="text-sm font-medium text-gray-500 dark:text-gray-400 capitalize border-b-2 block sm:pb-2 pb-1">
                  ...
                  >
                    ...
                    <SelectContent className="bg-white dark:bg-gray-600 max-h-48 overflow-auto">
                      {categories.length === 0 ? (
                        <SelectItem value="" disabled>No categories loaded</SelectItem>
                      ) : (
                        categories.map((cat) => (
              ...
              <div className="space-y-6">
                <div>
                  <Label htmlFor="date" className="text-sm font-medium text-gray-500 dark:text-gray-400 capitalize border-b-2 block pb-0">
                    Date
                  </Label>
                  <Input
                    type="date"
                ...
                <div className="md:col-span-1">
                  <Label htmlFor="description" className="text-sm font-medium text-gray-500 dark:text-gray-400 capitalize border-b-2 block pb-1">
                    Description
                  </Label>
                  <Textarea
                    id="description"
...
//         console.log("📦 Transaction raw:", tx);
...
//         console.log("✅ Category exists?", cats.some(c => String(c.id) === categoryId));
...
//         // SINGLE STATE UPDATE - everything together
//         setState({
//           formData: formattedTransaction,
//           categories: cats,
//           ready: true
//         });
...
//           categoryId,
//           hasMatch: cats.some(c => String(c.id) === categoryId),
...
//     setSaving(true);
//     try {
//       await api.updateTransaction(state.formData.id, {
//         amount: state.formData.amount,
//         category: state.formData.category || null,
...
//   if (!state.ready || state.categories.length === 0) {
//     return (
//       <div className="max-w-4xl p-6 border border-gray-300 dark:border-gray-600 dark:text-white bg-[#f5f6fb] dark:bg-[#0c1017] rounded-lg shadow-md shadow-gray-500 dark:shadow-gray-800 mx-auto sm:mt-8 mt-0">
//         <div className="flex justify-center items-center py-8">
//           Loading transaction data...
...
//         <div className="text-red-600 p-4 text-center">{error}</div>
//         <div className="flex justify-center mt-4">
//           <Button onClick={() => router.back()} className="px-4 py-2 rounded-lg border border-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700">
//             Go Back
//           </Button>
//         </div>
//       </div>
...
//   const currentCategoryName = state.categories.find(c => String(c.id) === String(state.formData!.category))?.name || "None";
...
//         <ItemContent>
//           <ItemTitle className="border-b-2 md:text-xl sm:text-lg text-md font-bold sm:pb-2 pb-1">
//             Edit Transaction ID: {state.formData!.id} 
...
//                 <div>
//                   <Label htmlFor="category" className="text-sm font-medium text-gray-500 dark:text-gray-400 capitalize border-b-2 block sm:pb-2 pb-1">
//                     Category ({state.categories.length} available) - Current: {currentCategoryName}
//                   </Label>
//                   <Select
//                     value={String(state.formData!.category)}
//                     onValueChange={(value) => handleSelectChange("category", value)}
//                   >
//                     <SelectTrigger className="mt-1 bg-white dark:bg-gray-600 border border-gray-300 dark:border-gray-500 text-gray-900 dark:text-gray-200 rounded-lg p-2 w-full">
//                       <SelectValue placeholder="Select category" />
...
//                         const catValue = String(cat.id);
//                         const isSelected = catValue === String(state.formData!.category);
//                         console.log(`Category option: "${catValue}" ${isSelected ? '✅ SELECTED' : '❌'}`);
//                         return (
//                           <SelectItem 
//                             key={catValue} 
//                             value={catValue}
